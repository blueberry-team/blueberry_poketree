...
import SpecialPoketmonHatchModal from "../Modal/SpecialPoketmonHatchModal";
...
export function SocialMediaButton({ onGetSpecialPokemon }: SocialMediaButtonProps = {}) {
  ...
  const openSocialMedia = useCallback(() => {
    const socialUrls = {
  ...
  const handleSocialClick = useMemo(
    () => debouncer.debounceLeading(async () => {
      // 스페셜 포켓몬 받기 함수가 전달되었으면 호출
      if (onGetSpecialPokemon) {
        try {
          await onGetSpecialPokemon();
          // API 호출 성공 후 SpecialPoketmonHatchModal 열기
          setIsSpecialPoketmonHatchModalOpen(true);
        } catch {
          return;
        }
      // 스페셜 포켓몬 받기 함수가 전달되지 않았으면 소셜미디어 열기
      } else {
        openSocialMedia();
      }
    }),
    [debouncer, onGetSpecialPokemon, openSocialMedia]
  );
  ...
  return (
    <>
      ...
      <SpecialPoketmonHatchModal
        isOpen={isSpecialPoketmonHatchModalOpen}
        onClose={handleSpecialPoketmonHatchModalClose}
        onComplete={handleSpecialPoketmonHatchModalComplete}
      />
    </>
  );
}
